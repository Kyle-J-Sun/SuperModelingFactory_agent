...
- **`woe_fit_scope` (post-0.6.6, G00)** — `"all"` (legacy default) vs `"post_missing_gate"`: the latter runs the selection-grade missing gate BEFORE the top-level WOE fit and feeds the gated survivor set to every WOE-consuming downstream stage (PSI/IVKS/corr/selection); gated features keep distribution diagnostics + a `missing_gate_dropped` table. **Announced flip to `"post_missing_gate"` next minor.**
- **Post-corr selection gates (post-0.6.6, G02–G06)** — via `selection_params`: `iv_upper_threshold` (leak band), `monthly_iv_min`/`monthly_iv_cv_max`/`direction_consistency_min` + `min_group_n`/`insufficient_group_policy` (group stability; needs `selection_group_dims` on the config), `target_rules`/`min_pass_count`/`per_target_iv_range`/`direction_reference_target` (multi-target), `max_selected_features`/`min_selected_features` (truncation, never backfills), `vif_enabled`/`vif_threshold`/`vif_min_features` (needs the `stats` extra). Gate order vif → group_stability → multi_target → truncation; evidence lives in `selection_summary["stage_tables"]`, per-feature drops in `"dropped_detail"`. G03/G04 thresholds outside FVP (no evidence) raise at `feature_screen` entry.
- **`monotone_woe_params` passthrough keys (post-0.6.6, G08/G09/G17)** — `min_bad_count`, `min_good_count`, `small_bin_policy` {merge,warn,raise}, `monotone_direction` (str|dict), `reference_target`, `direction_conflict_policy` {warn,raise,keep}, `missing_bin_strategy` {empirical_special,fixed_woe,fail}, `refine_min_n_bins_policy` {warn,enforce,raise} — all reach the underlying `MonotoneWOEBinner` from FVP, CMP, and `feature_screen`. Governance raises surface as `BinningPolicyViolation` (pierces per-feature fault tolerance). `refine_min_n_bins_policy` **flips to "warn" next minor**.
...
- **Evaluation governance (post-0.6.5)** — `eval_target_cols` (one frozen score, many labels; perf tables gain a `tgt_name` column only when set), `all_missing_score_value` (all-raw-missing rows get the override score; same test as `Core.scoring(all_missing_spec_value=...)`; persisted with `all_missing_raw_features` in artifact metadata), `special_score_values` (sentinels get their own gains row, excluded from quantile edges and AUC/KS — weighted and unweighted), `gains_ascending` (None = per-path legacy defaults; explicit bool unifies direction everywhere; **announced flip to True next minor**), `eval_weight_col` (`"inherit"`/None/str — decouple evaluation weights from training weights; also drives search/backward eval weights).
- **OOT governance (post-0.6.4)** — `candidate_mode` (one-switch preset: forbid OOT everywhere, never synthesize, evaluate ins/oos only, search oos only, backward reports nothing; contradicting explicit settings raise), `synthesize_missing_oot` (None resolves to legacy True with a loud "stand-in OOT" UserWarning; False drops the `"oot"` key entirely — announced flip to False next minor), `evaluation_splits` (whitelist for `_evaluate_models`; extra_eval_datasets exempt), `forbidden_splits` (hard gate at every consumption site incl. `optuna_params`/`backward_params` escape hatches), `search_eval_splits` + `search_objective_when_no_oot` (govern lr_search AND optuna eval_sets; explicit `oot_gap_penalized` without an available gap-ref raises), `backward_validation_split` / `backward_report_splits` (None = legacy `["oot"]`, flip to `[]` next minor). Check `result.split_governance` / saved model metadata (`oot_synthesized`, `oot_withheld`) to tell real OOT from placeholders. FVP has the matching `synthesize_missing_oot` (empty-frame representation instead of missing key).
